...
def insert_avatar_data_entries(queryHandler):
    define_avatar_records(queryHandler)
    insert_user_avatar_unlock_condition_records(queryHandler)
    insert_user_avatar_nickname_unlock_condition_records(queryHandler)
...
# insert records for avatar nickname unlock condition.
def insert_user_avatar_nickname_unlock_condition_records(queryHandler):
    avatar_keyword_combos = [
        # WAVE 1
        ('S1', ('jordo',)),
        ('S2', ('miku',)),
        ('S3', ('garfield',)),
        ('S4', ('samus', 'aran', 'metroid')),
        ('S5', ('boss', 'baby')),
        ('S6', ('white', 'walter')),
        # WAVE 2
        ('S7', ('pink', 'jesse')),
        ('S8', ('mike', 'ehrmantraut', 'finger')),
        ('S9', ('porky', 'pig')),
        ('S10', ('jason', 'vorhees', '13')),
        # WAVE 3
        ('S11', ('wild', 'mutt')),
        ('S12', ('rip', 'jaw')),
        ('S13', ('up', 'grade')),
        ('S14', ('jake', 'bacon', 'pancake')),
        ('S15', ('patrick', 'star')),
        ('S16', ('plankton', 'sheldon')),
        ('S17', ('tony', 'tiger', 'frosted', 'flakes')),
        ('S18', ('incredible', 'hulk', 'bruce', 'banner')),
        ('S19', ('woodstock',))
    ]

    for avatar in avatar_keyword_combos:
        avatar_id = avatar[0]
        keywords = avatar[1]
        for nickname_keyword in keywords:
            queryHandler.execute_query(TGOMMO_INSERT_NEW_AVATAR_NICKNAME_LINK, params=(avatar_id, nickname_keyword))
