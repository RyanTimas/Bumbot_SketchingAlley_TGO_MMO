...
class CatchButton(discord.ui.Button):
    ...
    def __init__(self, discord_bot, message: Message, database_handler: DatabaseHandler, creature:TGOCreature, environment:TGOEnvironment):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        successful_catch_embed = CreatureEmbedHandler(self.creature).generate_catch_embed(interaction=interaction, database_handler=self.database_handler)
        total_xp = successful_catch_embed[3]

        # insert record of user catching the creature
        self.database_handler.tgommo_database_handler.insert_new_user_creature(params=(interaction.user.id, self.creature.creature_id, self.creature.variant_no, self.environment.environment_id, self.creature.rarity == MYTHICAL))
        # give user xp for catching the creature
        self.database_handler.user_database_handler.update_xp(total_xp, interaction.user.id, interaction.user.display_name)
...
class TGOMMOCatchButtonView(View):
    def __init__(self, discord_bot, message, database_handler, creature:TGOCreature, environment:TGOEnvironment):
        super().__init__(timeout=None)
        self.add_item(CatchButton(discord_bot=discord_bot, message=message, database_handler=database_handler, creature=creature, environment=environment))