...
from src.resources.constants.TGO_MMO_constants import FONT_COLOR_WHITE, ITEM_INVENTORY_EXCLUDED_ITEM_TYPES, ITEM_INVENTORY_ICON_ORDER_MAP
...
class ItemInventoryImageFactory(BaseImageFactory):
    def __init__(self, message_author, target_user):
        ...
        self.active_tab = next(iter(ITEM_INVENTORY_ICON_ORDER_MAP))
        ...
        self.active_items = [item for item in self.user_items if item.item_type in  ITEM_INVENTORY_ICON_ORDER_MAP.get(self.active_tab)]
    ...
    def get_inventory_categories_grid(self, image: Image):
        icons = []
        for icon in ITEM_INVENTORY_ICON_ORDER_MAP.keys():
            icon_image = Image.open(icon)

            # reduce opacity to 35%
            if icon != self.active_tab:
