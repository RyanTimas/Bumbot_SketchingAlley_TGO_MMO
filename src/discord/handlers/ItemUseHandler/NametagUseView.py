...
class NametagUseView(View):
    def __init__(self, user: TGOPlayer, item_use_handler):
        super().__init__(timeout=300)
        self.user = user
        self.nametag_item = get_tgommo_db_handler().get_inventory_item_by_user_id_and_item_id(user_id=user.user_id, item_id=ITEM_ID_NAMETAG)
        ...
        self.add_item(create_display_creature_collection_button(user=user, row=0))
    ...
    async def rename_creature_modal_submit_callback(self, interaction):
        try:
            ...
            if not get_tgommo_db_handler().does_user_own_catch_id(user_id=self.user.user_id, catch_id=catch_id):
            ...
            get_tgommo_db_handler().update_user_profile_available_items(user_id=self.user.user_id, item_id=ITEM_ID_NAMETAG, new_amount=get_tgommo_db_handler().get_inventory_item_by_user_id_and_item_id( item_id=self.nametag_item.item_id, user_id=self.user.user_id).item_quantity - 1)
            ...
            await interaction.followup.send(f"<@{self.user.user_id}> ({self.user.nickname}) used a nametag to rename their creature!", files=[self.item_use_handler.get_image_for_item(self.nametag_item)])
        except ValueError:
